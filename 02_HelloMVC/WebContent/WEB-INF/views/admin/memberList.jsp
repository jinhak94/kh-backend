	...
	*/
	$("select.member-role").change(function(){
		...
		var bool = confirm("[" + memberId + "]회원의 권한을 [" 
			+ (memberRole ==  "U"? "일반" : "관리자") + "](으)로 변경하시겠습니까?");
		if(!bool){
			location.reload();			
			return;
		}
		...
		/* location.href = "/mvc/admin/updateMemberRole?memberId=s&memberRole=U";
			 -> Get방식. 입력값이 노출되면 안되는 것은 Post로 바꿔도 됌.(아래 경우 아니더라도)
		// GET : 요청처리이후 DB에 변경내역이 없는 경우. 조회|검색
		...
		<% } else { 
			...
			<tr>
				...
				<td>
					...
					<%-- 데이터속성 : 태그에 data를 직접 저장/불러오기 가능 --%>
					<select id="" class="member-role" data-member-id="<%=m.getMemberId()%>">
				...
				<td><%= "M".equals(m.getGender()) ? "남" : ("F".equals(m.getGender()) ? "여" : "") %></td>
